...
if (!isQuotedSticker) return reply('Reply stiker nya')
					if (mek.message.extendedTextMessage.con.textInfo.quotedMessage.stickerMessage.isAnimated === true){
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							nayla.sendMessage(from, buffer, image, {quoted: mek, thumbnail:virgam, caption: 'Nih Fotonya\n\n⚠️*Perhatian.!!*⚠️\nFoto ini Mengandung Virgam/Bug SW!!'})
							fs.unlinkSync(ran)
						})
					}
					break
...
default: 
if (budy.match(`ontol`)) {
nayla.sendMessage(from,{ audio: { url: `media/audio/gaboleh.m4a`},ptt:true, mimetype: 'audio/mp4'}, {quoted:{key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: from } : {})},message: {"extendedTextMessage": {"text": `HY : ${pushname}\n❀ RUNTIME : ${kyun(runtime)}`, 'title':'2', 'jpegThumbnail': fs.readFileSync(`./media/image/audio.jpg`)}}}})
}
if (budy.match(`olol`)) {
nayla.sendMessage(from,{ audio: { url: `media/audio/gaboleh.m4a`},ptt:true, mimetype: 'audio/mp4'}, {quoted:{key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: from } : {})},message: {"extendedTextMessage": {"text": `HY : ${pushname}\n❀ RUNTIME : ${kyun(runtime)}`, 'title':'2', 'jpegThumbnail': fs.readFileSync(`./media/image/audio.jpg`)}}}})
}
